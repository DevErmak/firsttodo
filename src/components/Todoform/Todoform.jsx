...
export default function Todoform() {
  ...
  return (
    ...
    <input value={inputTask} type='text' onKeyUpCapture={(e)=>addTaskEnter(e)}
    placeholder='введите задачу'
    onChange={(e)=>setInputTask(e.target.value)}></input>
    <button onClick={()=>addTask()}>Записать</button>
    <button onClick={()=>findTasks()}>{isSearch && inputTask.trim() !== '' ? "закрыть поиск" : "поиск"}</button>
    <button onClick={()=>removeGroupTasks()} 
    style={{visibility: groupTasks.length > 1 ? null : 'hidden'  }}>удалить группу элементов</button>

    { isSearch && inputTask.trim() !== '' ? foundTasks === [] ? console.log(isSearch): foundTasks.map(foundTask => <Task key={foundTask.id} task={foundTask} changeTodo={changeTodo} removeOneTask={removeOneTask}/>) :
      tasks.map(task => <Task key={task.id} task={task} changeTodo={changeTodo} removeOneTask={removeOneTask}/>)}
